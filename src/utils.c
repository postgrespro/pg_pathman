...
get_pathman_attributes_map(const PartRelationInfo *prel, Relation child)
{
	...
	TupleDesc	childDesc = RelationGetDescr(child);
	int			natts = childDesc->natts;
	AttrNumber *attrMap = (AttrNumber *) palloc0(natts * sizeof(AttrNumber));
	...
	while ((i = bms_next_member(prel->expr_atts, i)) >= 0)
	{
		...
		for (j = 0; j < natts; j++)
		{
			...
			child_attname = NameStr(att->attname);
			if (strcmp(child_attname, attname) == 0)
			{
				attrMap[i - 1] = (AttrNumber) (j + 1);
				break;
			}
		}
